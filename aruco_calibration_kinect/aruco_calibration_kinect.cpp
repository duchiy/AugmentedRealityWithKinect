...
void setCurrentViewPoint(vector< cv::Point3f > &objPoints, vector< cv::Point2f > &imagePoints) {
    ...
    if (minDistanceToStoredLocations(Tvec) > ViewPointDistance) {
        ...
        if (TheobjPointsV.size() >= 3 && TheobjPointsV.size() < 8) {
            ...
            float repro = cv::calibrateCamera(TheobjPointsV, TheimagePointsV, TheCamParams.CamSize, TheCamParams.CameraMatrix, TheCamParams.Distorsion,
                                              The_rvecs, The_tvecs);
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        while (TheVideoCapturer.grab() && key != 27) { //
            ...
            // detect
//			Mat gray_image;
//			cvtColor( TheInputImage, gray_image, COLOR_BGR2GRAY );
//			imwrite( "Gray_Image.jpg", gray_image );
//			imwrite( "Color_Image.jpg", TheInputImage );

			float prob = TheBoardDetector.detect(TheInputImage);
			int size=TheBoardDetector.getDetectedBoard().size();
